defmodule TodoListTest do
  ...
  test "delete_entry" do
    ...
    assert(0 == deleted_list |> Todo.List.entries({2016, 12, 20}) |> length)
  end

  defp sample_todo_list do
    Todo.List.new
    |> Todo.List.add_entry(%{date: {2016, 12, 19}, title: "Dentist"})
    |> Todo.List.add_entry(%{date: {2016, 12, 20}, title: "Shopping"})
    |> Todo.List.add_entry(%{date: {2016, 12, 19}, title: "Movies"})
  end

  defp shopping_entry(todo_list \\sample_todo_list()) do
    todo_list
    |> Todo.List.entries({2016, 12, 20})
    |> Enum.at(0)
  end
end
